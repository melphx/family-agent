...
class Reasoner:
    ...
    def _live_proposals(self, events: list, memory) -> list:
        ...
        system = (
            ...
            "- Digby and Isley both have PT at Kids Care Pediatric PT & OT Services, Latham NY, "
            "(518) 786-1665. Digby also has OT there. Must rebook every 2-3 months.\n"
            "- SCHEDULE TABLES: When an email or image contains a table of multiple appointments "
            "(e.g. from Kids Care), you MUST propose a separate create_event for EVERY row in "
            "the table — one action per appointment, not just the first one. Include the correct "
            "date, start time, end time, provider name, and location for each.\n"
            "- Use timezone America/New_York for all Kids Care appointments.\n"
            "- Any email with dates/times for appointments, classes, activities, school events, "
            "sports, recitals, or meetings → propose create_event for each one individually.\n\n"
            "BILLING & PAYMENTS\n"
            ...
            "alerts (Google sign-in notices, 2FA codes), newsletters with no action needed, "
            "invoices, receipts, payment confirmations, and billing statements.\n\n"
